...
**The server holds the long-lived credential; the client never sees it.** The client secret and the rotating refresh token stay on the server. What the client does receive is the minted chat URL, which carries a short-lived bearer credential of its own — see below for what that means for where it may be stored or forwarded. Restream's OAuth offers no PKCE and its token exchange requires a client secret, which an application distributed to operators cannot keep. Restream's own documentation directs integrations to keep that secret off user devices and to refresh through a proxy the application provides — the server is that proxy.
...
**These routes inherit §1's posture: there is no in-protocol auth.** Anything that can reach the listener can ask for a chat URL, which is a live credential for the operator's chat account — a step up from the button presses the rest of v1 exposes, and another reason the listener belongs on a trusted network. A host that can reach the listener can also run the authorization handshake itself and replace the stored credential with one of its own, and then post into the chat the operator is watching on a screen. Nothing in v1 prevents that; the network isolation §1 assumes is what stands between them.

**Two bounds narrow that, because a credential is worth more than a button press.** Neither is authentication:

- **These routes answer only on an address `public_url` names.** It takes a list — a tailnet name, a LAN address, `localhost` at the production PC — and chat answers on any of them; the first is the one the OAuth handshake uses, because the platform matches the redirect URI exactly. `/ws` compares `Origin` against `Host`, which is a self-consistency check on two headers the requesting page supplies — so a page that has made this server's address into its own name (DNS rebinding) satisfies it, and is same-origin by the browser's own rules. That is enough to reach an API that presses buttons in a room; it is not enough to hand out a credential that keeps working from anywhere afterwards. Comparing `Host` against the address the operator configured is what the same-origin check cannot do for itself.
- **A browser that says a request came from another site is refused,** and starting an authorization is rate-limited. An absent `Sec-Fetch-Site` is allowed — a native client sends none — so this narrows what a hostile *page* can do, not what a host on the network can do.

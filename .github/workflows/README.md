...
| `scripts/` | `shellcheck scripts/*.sh`, `scripts/devstack-test.sh` | The dev-stack tests start no containers, so this needs nothing beyond the runner's bash and python3. Also triggered by `server/**`, because `devstack.sh` parses the server's log messages and the tests assert those messages still exist, and by `docs/development.md`, because they assert §8 shows every client state `status` can print. |
...
The per-PR/push CI workflows are implemented (`server.yml`, `client.yml`, `sidecar.yml`) — landed in #68 (CB-003) — plus `scripts.yml` (CB-094). The release/installer builds are not yet scaffolded; they're tracked in [CB-087](https://github.com/cuebooth/cuebooth/issues/71) and will land alongside the first version-tagged release.
